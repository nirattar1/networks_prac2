...
int initiate_server(unsigned short int port, char usersFilePath[]){ 
	...
	if ((check_select < 0) && (errno!=EINTR)){
	   ...
	   return STATE_ERROR;
        ...
        if (FD_ISSET(server_sock, &read_fds)){ /* server take care of a new client */
	   ...
	   if (client_sock < 0){
		...
		for (i = 0; i < MAX_CLIENTS; i++){ //check what happen is the array if full              
		...
		if (state_flag == STATE_ERROR){ //error - close client sock - check if it's enough
		    printf("check for me");
      ...
      for (i = 0; i < MAX_CLIENTS; i++){ //check for every client socket
            fd = client_array[i].socket_fd;
            	 
	    if (FD_ISSET(fd ,&read_fds)){ //means the client sent something to server 
	      state_flag = server_to_client_new(fd, usersFilePath);
	      if (state_flag == STATE_ERROR){
		  continue;	  //check wht to do in case of ERROR
	      }
            }
      }
